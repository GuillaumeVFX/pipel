...
if app.get_selection().get_count() > 0:
    ...
    if item.is_context():
        ...
        if context_selected != 0:
            title = "Export Context As Project..."
            ...
            if filename != "":
                ...
                app.disable()
                ix.reference_export_context(context_selected, filename)
                app.enable()
                clarisse_win.set_mouse_cursor(ix.api.Gui.MOUSE_CURSOR_DEFAULT)
else:
    ix.log_warning("Export Context As Reference error: Works only a context.")
