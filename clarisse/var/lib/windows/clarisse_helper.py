...
def export_context_as_project(context, filename):
    return api.IOHelpers.export_context_as_project(application, context, filename)
...
def export_context_as_project_with_dependencies(context, filename):
    return api.IOHelpers.export_context_as_project_with_dependencies(application, context, filename)

## Export a context to the specified project filename, and convert the context to a reference
# context that refernces the exported project.
# @param app Application object
# @param context Context
# @param filenames project filename
# @return True on success, False on failure
def reference_export_context(context, filename):
    return api.IOHelpers.reference_export_context(application, context, filename)
